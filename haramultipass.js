...
var haramultipass = module.exports = function(secret) {
    if (!(this instanceof haramultipass)) return new haramultipass(secret);
...
haramultipass.prototype.encode = function(obj) {
    if (!obj) return;
...
haramultipass.prototype.generateUrl = function(obj, domain) {
    if(!domain) return;
    return "https://" + domain + "/account/login/multipass/" + this.encode(obj);
};

haramultipass.prototype.sign = function (data) {
    var signed = crypto.createHmac("SHA256", this._signingKey).update(data).digest();
    return signed;
}

haramultipass.prototype.encrypt = function(plaintext) {
    // Use a random IV
